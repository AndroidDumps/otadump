...
impl<'a> BitReader<'a> {
    pub fn new(buf: &'a [u8]) -> Self {
        BitReader { buf, index: 0, cache: 0, cache_bits: 0 }
    }
    ...
    pub fn offset_in_bits(&self) -> Result<usize> {
        self.index
            .checked_mul(8)
            .and_then(|offset| offset.checked_sub(self.cache_bits))
            .ok_or_else(|| Error::Corrupt("bit reader position is invalid".into()))
    }
...
impl<'a> BitWriter<'a> {
    pub fn new(out: &'a mut [u8]) -> Self {
        BitWriter { out, index: 0, holder: 0, holder_bits: 0 }
    }
    ...
    pub fn offset_in_bits(&self) -> Result<usize> {
        self.index
            .checked_mul(8)
            .and_then(|offset| offset.checked_add(self.holder_bits))
            .ok_or_else(|| Error::Corrupt("bit writer position overflows".into()))
    }
}
